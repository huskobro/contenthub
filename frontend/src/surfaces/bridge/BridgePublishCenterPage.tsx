...
export function BridgePublishCenterPage() {
  ...
  const columns = useMemo(() => {
    return BOARD_COLUMNS.map((col) => ({
      ...col,
      records: items.filter((r) => col.statuses.includes(r.status)),
    }));
  }, [items]);
    ...
    <div className="flex flex-col gap-3" data-testid="bridge-publish-center">
      ...
      <div className="flex items-baseline justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-neutral-500" data-testid="bridge-publish-total">
            Toplam: {total}
          </span>
        </div>
      </div>
        ...
        >
          {columns.map((col) => (
            ...
            >
              ...
              <div className="p-2 flex flex-col gap-1.5 min-h-[120px] max-h-[calc(100vh-260px)] overflow-y-auto">
                {col.records.length === 0 && (
                  <p className="m-0 text-[11px] text-neutral-400 px-1">bos</p>
                )}
                {col.records.map((r) => (
                  <button
                    key={r.id}
