...
@pytest.mark.asyncio
async def test_list_blocks_empty(client: AsyncClient, admin_headers: dict):
    """Before seed, may return seeded blocks."""
    resp = await client.get(f"{BASE}/blocks", headers=admin_headers)
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)


@pytest.mark.asyncio
async def test_create_block(client: AsyncClient, admin_headers: dict):
    payload = {
        "key": "test.block.api",
    ...
    resp = await client.post(f"{BASE}/blocks", json=payload, headers=admin_headers)
...
@pytest.mark.asyncio
async def test_update_block_admin_override(client: AsyncClient, admin_headers: dict):
    # Create first
...
@pytest.mark.asyncio
async def test_protected_block_cannot_disable(client: AsyncClient, admin_headers: dict):
    create_resp = await client.post(f"{BASE}/blocks", json={
...
@pytest.mark.asyncio
async def test_preview_dry_run(client: AsyncClient, admin_headers: dict):
