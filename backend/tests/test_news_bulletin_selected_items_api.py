...
def test_news_bulletin_selected_items_table_exists():
    from app.core.config import settings
    db_path = settings.database_url.replace("sqlite+aiosqlite:///", "")
    conn = sqlite3.connect(db_path)
    cur = conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='news_bulletin_selected_items'"
    )
    row = cur.fetchone()
    conn.close()
    assert row is not None, "news_bulletin_selected_items table should exist after migration"
