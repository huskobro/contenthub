...
class BulletinScriptExecutor(StepExecutor):
    ...
    async def execute(self, job: Job, step: JobStep) -> dict:
        ...
        persisted_script = await persist_script_row(
            bulletin_id=bulletin_id,
            script_data=script_data,
            notes=f"Generated by job {job.id}",
        )
        if not persisted_script:
            logger.warning(
                "BulletinScriptExecutor: NewsBulletinScript persistence skipped "
                "for bulletin=%s job=%s", bulletin_id, job.id,
            )
